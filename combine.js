	...
	*/
	combineFunction: function(formulas, geo, Tformulas, rfuncs){
		...
		array.forEach(this.colors, lang.hitch(this,function(cColor, i){
			items = items + '<div class="item" style="margin-bottom: 3px;"> <div style="width: 15px; 		color-adjust: exact;  -webkit-print-color-adjust: exact; print-color-adjust: exact; margin-right: 5px; display: inline-block; height: 15px; border: 1px solid black; background-color:rgb('+ cColor[1] + "," + cColor[2] + "," + cColor[3] + ');" ></div>' + '<span class="item-label" style="color: black; display: inline; line-height: 15px; vertical-align: top;">' + this.labels[i] + '</span></div>'
		}));
		...
		var explorerJSON = JSON.parse(explorer);
